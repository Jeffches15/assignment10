...
def pytest_addoption(parser):
    ...
    # this adds a custom flag in the pytest command
    # The test database is not recreated between test runs.
    # So if you ran the test once and created data (like users), it's still there.
    # Your tests pass because the required tables and data are already present.
    parser.addoption(
        "--preserve-db",
        action="store_true",
        default=False,
    ...
    # Without --preserve-db:
    # The test framework resets or drops the DB before or after the test run.
    # If your test doesn’t recreate the schema or insert necessary test data, it fails because:
